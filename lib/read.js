...
var Read = Juttle.proc.source.extend({
    ...
    handleTimeOptions: function(options) {
        ...
        if (options.from) {
            ...
            date = new Date(options.from.valueOf());
            this.client.start(date.valueOf());
            logger.debug('start time', date.toISOString());
        }
        if (options.to) {
            this.to = options.to;
            date = new Date(options.to.valueOf());
            this.client.end(date.valueOf());
            logger.debug('end time', date.toISOString());
        }
    },
    //Query execution
    ...
    start: function() {
        ...
        function executeQuery() {
            ...
            return new Promise(function(resolve, reject) {
                self.client.get(function(err, result) {
                    if (err) {
                        reject(err);
                    }
                    resolve(result);
